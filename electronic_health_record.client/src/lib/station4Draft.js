// Station 4 dental drafts.
//
// A copy of station3Draft.js rather than a shared abstraction: the two stations
...
const PREFIX = 'ehr:station4-draft:';
const VERSION = 1;
...
/**
 * Persist an in-progress dental assessment.
 * @returns {string|null} ISO timestamp of the save, or null if it failed.
 */
export function saveDraft(formID, { values, signature, dentistID = null }) {
  if (!formID) return null;
  const savedAt = new Date().toISOString();
  try {
    localStorage.setItem(
      keyFor(formID),
      JSON.stringify({ version: VERSION, savedAt, values, signature, dentistID }),
    );
    return savedAt;
  } catch {
 ...
 *   dentistID: number|null|undefined}|null}
 */
