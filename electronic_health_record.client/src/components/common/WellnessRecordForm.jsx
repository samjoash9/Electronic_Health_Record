...
export default function WellnessRecordForm({ phoData, onCancel, onSave }) {
    ...
    useEffect(() => {
        ...
        setIsLoading(true);
        axios.get(`https://localhost:7165/api/WellnessForms/${patientId}`)
            .then(response => {
    ...
    const submitForm = async (targetStatus) => {
        ...
        try {
            if (isManualEntry || isNewForm || !patientId) {
                const response = await axios.post(`https://localhost:7165/api/WellnessForms`, payload);
                ...
                await axios.put(`https://localhost:7165/api/WellnessForms/${formId}`, payload);
