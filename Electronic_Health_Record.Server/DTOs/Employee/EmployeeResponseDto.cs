namespace Electronic_Health_Record.Server.DTOs.Employee
{
    /// <summary>
    /// Local employee directory record as served to the client. Field names
    /// mirror <see cref="UpsertEmployeeDto"/> so the Onboarding page's list and
    /// edit form share one shape.
    /// </summary>
    public class EmployeeResponseDto
    {
        public string ExternalEmployeeId { get; set; } = string.Empty;
        public string Surname { get; set; } = string.Empty;
        public string FirstName { get; set; } = string.Empty;
        public string? MiddleName { get; set; }
        public DateTime Birthdate { get; set; }
        public string Sex { get; set; } = string.Empty;
        public string CivilStatus { get; set; } = string.Empty;
        public string? Address { get; set; }
        public string? AgencyOffice { get; set; }
        public string? Position { get; set; }
        public string? ContactNo { get; set; }
        public bool IsLocallyAdded { get; set; }
    }
}
